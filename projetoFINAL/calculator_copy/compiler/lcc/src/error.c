...
static void prterm(int n) {
	static char *terms[] = { "NIL",
/* 1 */ "CNST",  "ARG", "ASGN", "INDIR", "?5?", "?6?", "CVF", "CVI", "CVP",
/* 10 */ "?10?", "CVU", "NEG", "CALL", "LOAD", "RET", "ADDRG", "ADDRF",
/* 18 */ "ADDRL", "ADD", "SUB", "LSH", "MOD", "RSH", "BAND", "BCOM", "BOR",
/* 27 */ "BXOR", "DIV", "MUL", "EQ", "GE", "GT", "LE", "LT", "NE", "JUMP",
/* 36 */ "LABEL", "?38?", "?39?", "?40?", "?41?", "?42?", "?43?", "VREG", 0 };
	fprintf(stderr, "\tterminal %d = %s%c%d not supported.\n",
		n, terms[n>>4 & 0x3F], "FIUPVB"[n&7], n>>10);
}
...
int fatal(const char *name, const char *fmt, int n) {
	...
	fprint(stderr, fmt, n);
	if (!strncmp(fmt, "Bad terminal ", 13)) prterm(n);
	exit(EXIT_FAILURE);
	return 0;
}
